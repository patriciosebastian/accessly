...
export async function auditHTML(filePath) {
  logVerbose(chalk.blue(`Reading file: ${filePath}`));
  ...
  logVerbose(chalk.blue(`Audit completed with ${results.length} issues`));
  return results;
}

// Add annotations directly into the HTML file
export async function annotateHTML(filePath, results) {
  logVerbose(chalk.blue(`Annotating file: ${filePath}`));
  const html = await fs.readFile(filePath, 'utf8');
  const lines = html.split('\n');
  ...
  logVerbose(chalk.blue('Annotations Completed.'));
  return lines.join('\n');
}
