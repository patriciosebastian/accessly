...
export async function initCommand(options = {}) {
  ...
  if (exists && !options.force) {
    ...
    if (!overwrite) {
      logDefault(chalk.yellow('Initialization aborted.'));
      return;
    }
  }
  ...
  if (options.environment || options.apiEndpoint || options.logLevel || options.reportFormat || options.ciMode !== undefined) {
    ...
    logVerbose(chalk.blue('Non-interactive mode: Using provided configuration options'));
  } else {
    // Interactive mode
  ...
  try {
    ...
    logDefault(chalk.green(`Configuration saved to ${DEFAULT_CONFIG_PATH}`));
    // logVerbose(chalk.blue(`Configuration details: ${JSON.stringify(config, null, 2)}`));
  } catch (err) {
    logError(chalk.red(`Error saving config: ${err.message}`));
    logVerbose(chalk.red(`Detailed error: ${err.stack}`));
  }
}
